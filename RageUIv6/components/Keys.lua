...
function Keys.Register(Controls, ControlName, Description, Action)
    ...
    RegisterKeyMapping(string.format('RageUIv6-%s', ControlName), Description, "keyboard", Controls)
    RegisterCommand(string.format('RageUIv6-%s', ControlName), function(source, args)
        if (Action ~= nil) then
            print(string.format('RageUIv6 - Pressed keys %s', Controls))
            Action();
        end
    end, false)
